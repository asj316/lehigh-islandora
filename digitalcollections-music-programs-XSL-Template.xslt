<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
  ...
  <xsl:param name="Year"/>
  <xsl:param name="Performance-Dates"/>
  ...
  <xsl:param name="Performance-Location"/>
  ...
  <xsl:template name="root">
    <mods xmlns="http://www.loc.gov/mods/v3"
      ...
      <xsl:if test="string-length($Director)">
        <xsl:for-each select="tokenize($Director, ' ; ')">
          <name>
      ...
      <xsl:if test="string-length($Performer)">
        <xsl:for-each select="tokenize($Performer, ' ; ')">
          <name>
            <namePart><xsl:value-of select="normalize-space(.)"/></namePart>
            <role>
              <roleTerm type="text" authority="marcrelator">Performer</roleTerm>
              <roleTerm type="code" authority="marcrelator">prf</roleTerm>
            </role>
          </name>
        </xsl:for-each>  
      </xsl:if>
      <xsl:if test="string-length($Presenter)">
        <xsl:for-each select="tokenize($Presenter, ' ; ')">
          <name type="corporate">
            <namePart><xsl:value-of select="normalize-space(.)"/></namePart>
            <role>
              <roleTerm type="text" authority="marcrelator">Presenter</roleTerm>
              <roleTerm type="code" authority="marcrelator">pre</roleTerm>
            </role>
          </name>
        </xsl:for-each>  
      </xsl:if>
      <xsl:if test="string-length($Composer)">
        <xsl:for-each select="tokenize($Composer, ' ; ')">
          <name>
            <namePart><xsl:value-of select="normalize-space(.)"/></namePart>
            <role>
              <roleTerm type="text" authority="marcrelator">Composer</roleTerm>
              <roleTerm type="code" authority="marcrelator">cmp</roleTerm>
            </role>
          </name>
        </xsl:for-each>  
      </xsl:if>
      ...
      <xsl:if test="string-length($Performance-Dates)">
          <xsl:for-each select="tokenize($Performance-Dates, ' ; ')">
            <dateOther type="performance-date"><xsl:value-of select="normalize-space(.)"/></dateOther>
          </xsl:for-each>
      </xsl:if>
      ...
      <xsl:if test="string-length($Musical-Works)">
        <tableOfContents type="Musical Works"><xsl:value-of select="normalize-space($Musical-Works)"/></tableOfContents>
      </xsl:if>
      ...
      <xsl:if test="string-length($Performance-Location)">
        <xsl:for-each select="tokenize($Performance-Location, ' ; ')">
      ...
      <xsl:if test="string-length($Bepress-Identifier)">
        <identifier type="call-number"><xsl:value-of select="normalize-space($Bepress-Identifier)"/></identifier>
      </xsl:if>
