<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
  ...
  <xsl:param name="Creator-Corporate"/>
  ...
  <xsl:template name="root">
    <mods xmlns="http://www.loc.gov/mods/v3"
      ...
      <xsl:if test="string-length($Creator-Corporate)">
        <xsl:for-each select="tokenize($Creator-Corporate, ' ; ')">
          <name type="corporate">
      ...
      <xsl:if test="string-length($Advisors)">
        <name>
          <namePart>
            <xsl:value-of select="normalize-space($Advisors)"/>
          </namePart>
          <role>
      ...
      <xsl:if test="string-length($Embargo-Date)">          
        <dateValid encoding="iso8601" qualifier="embargo"><xsl:value-of select="normalize-space($Embargo-Date)"/></dateValid>
      </xsl:if>
      ...
      <xsl:if test="string-length($Report-Number)">
        <identifier type="report-number" displayLabel="Report Number"><xsl:value-of select="normalize-space($Report-Number)"/></identifier>
      </xsl:if>
